...
create table target (
  id serial primary key,
  categoryCode text references category(code),
...
create table observation (
  ...
  timestamp timestamp,
  value integer,
  primary key (trialId, observationTypeCode)
);
...
create table target_goal (
  targetId integer primary key references target(id),
  goal text
);
